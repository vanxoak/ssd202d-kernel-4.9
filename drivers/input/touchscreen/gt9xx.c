...
s32 gtp_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
    ...
    while (retry++ < 3)
    {
        ...
        buf[0] = (u8)(addr >> 8);
        buf[1] = (u8)(addr & 0xFF);
        gtp_i2c_read(client, buf, len + 2);///报错

        memset(confirm_buf, 0xAB, 16);
        confirm_buf[0] = (u8)(addr >> 8);
        confirm_buf[1] = (u8)(addr & 0xFF);
        gtp_i2c_read(client, confirm_buf, len + 2);///报错
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ret = gtp_i2c_test(client);//i2c报错问题
    if (ret < 0)
    {
    ...
    ret = gtp_request_input_dev(ts);//创建dev/input/设备
    if (ret < 0)
    {
    ...
    {
        GTP_INFO("GTP works in interrupt mode.");//成功
    }
