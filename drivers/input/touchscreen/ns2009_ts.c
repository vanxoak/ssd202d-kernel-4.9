...
#else
#define NS2009_READ_X_LOW_POWER_12BIT	0xdc
#define NS2009_READ_Y_LOW_POWER_12BIT	0xcc
#endif
...
#else
#define NS2009_DEF_X_FUZZ	16
#define NS2009_DEF_Y_FUZZ	32
#endif
...
//	}
	if (ret) {
	//	printk("[z1:%d]:dev->poll_interval:%d\n",z1,dev->poll_interval);
		//return ret;
		flag = 1;
		return ;
	}
	...
	if (z1 >= NS2009_PEN_UP_Z1_ERR) {
		...
		input_report_abs(data->input,ABS_X,abs(MAX_12BIT-x));
		input_report_abs(data->input,ABS_Y,abs(MAX_12BIT-y));
		//printk("x, y: %d, %d\n", abs(MAX_12BIT-x), abs(MAX_12BIT-y));


		input_sync(data->input);
	//	printk("[input]:dev->poll_interval:%d\n",dev->poll_interval);
	} else if (data->pen_down) {
		input_report_key(data->input, BTN_TOUCH, 0);
		input_sync(data->input);
	//	printk("[pen_down:false]:dev->poll_interval:%d\n",dev->poll_interval);	
		data->pen_down = false;
	}
	//return 0;
//	if(flag == 1){
//	printk("func finnish [%d]\n",z1);
//	flag=0;
//	}
	return ;
}
